...
### 核心功能
- 🔍 **智能解析**：自动识别并提取文件码（支持 `@filepan_bot:xxx` 和 `showfilesbot_xxx` 格式）
- 📝 **列表管理**：编辑备注、一键复制、删除记录
- 💾 **数据持久化**：使用 Cloudflare Workers KV 存储数据
- 🔐 **访问控制**：密码保护，SHA-256 加密验证
- 🎨 **现代 UI**：深色主题，响应式设计，优雅的用户体验
...
### 解析文本
1. 在"粘贴文本进行解析"区域输入包含文件码的文本
2. 点击"解析并添加"
3. 系统自动识别并提取所有文件码

### 管理文件码
- **编辑备注**：点击"编辑"按钮修改备注信息
- **复制文件码**：点击"复制"按钮复制到剪贴板
- **删除记录**：点击"删除"按钮（需二次确认）
- **手动添加**：使用"手动添加文件码"表单
